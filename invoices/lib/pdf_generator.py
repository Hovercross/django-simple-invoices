...
from invoices.lib.pdf_styles import (
    accent_color,
    line_item_date_format,
    table_item_style,
    table_item_style_right,
    table_header_style,
    table_header_style_right,
    table_name_style,
    default_table_style,
    table_total_style,
)
...
class InvoicePDFBuilder(object):
    ...
    def on_first_page(self, canvas=None):
        ...
        canvas.line(0.75 * inch, (11 - 0.5) * inch, 7.75 * inch, (11 - 0.5) * inch)
        canvas.drawString(0.75 * inch, (11 - 0.75) * inch, self.invoice.vendor.name)

        canvas.setFont("HelveticaNeue-Light", 16)
        canvas.setFillColor(colors.black)
        canvas.drawString(0.75 * inch, 9.75 * inch, "INVOICE")

        line_space = 0.15 * inch
        ...
        if self.invoice.vendor.phone:
            canvas.drawString(0.75 * inch, left_side_y, self.invoice.vendor.phone)
            left_side_y -= line_space

        if self.invoice.vendor.email:
            canvas.drawString(0.75 * inch, left_side_y, self.invoice.vendor.email)
            left_side_y -= line_space
        ...
        if self.invoice.vendor.address:
            ...
            for address_line in address_lines:
                canvas.drawString(0.75 * inch, left_side_y, address_line)
                left_side_y -= line_space
        ...
        if self.invoice.date:
            canvas.drawString(
                4.25 * inch,
                right_side_y,
                "Invoice date: {:}".format(self.invoice.date.strftime("%B %d, %Y")),
            )
            right_side_y -= line_space

        canvas.drawString(
            4.25 * inch, right_side_y, "Invoice number: {:}".format(self.invoice.id)
        )
        ...
        if self.invoice.vendor.checks_payable_to:
            canvas.drawString(
                4.25 * inch,
                right_side_y,
                "Please make checks payable to {}".format(
                    self.invoice.vendor.checks_payable_to
                ),
            )
            right_side_y -= line_space
        ...
        canvas.line(0.75 * inch, top_section_y, 7.75 * inch, top_section_y)
            ...
            if not uniform_rate:
                table.append(
                    [
                        hourly_service.date
                        and Paragraph(
                            hourly_service.date.strftime(line_item_date_format),
                            table_item_style,
                        )
                        or None,
                        hourly_service.location
                        and Paragraph(hourly_service.location, table_item_style)
                        or None,
                        hourly_service.description
                        and Paragraph(hourly_service.description, table_item_style)
                        or None,
                        hourly_service.duration
                        and Paragraph(
                            format_duration(hourly_service.duration),
                            table_item_style_right,
                        )
                        or None,
                        hourly_service.rate
                        and Paragraph(
                            "{:0.2f}".format(hourly_service.rate),
                            table_item_style_right,
                        )
                        or None,
        ...
        if uniform_rate:
            ...
            table.append(
                [
                    ...
                    None,
                    Paragraph(
                        "${:.2f}".format(self.invoice.hourly_services_total),
                        table_total_style,
                    ),
                ]
            )
            ...
            table_style.remove(("LINEABOVE", (0, 0), (-1, -2), 0.25, colors.grey))
            table_style.append(("LINEABOVE", (0, -3), (-1, -3), 1, colors.black))
            table_style.append(("LINEABOVE", (0, 0), (-1, -4), 0.25, colors.grey))
    ...
    def get_fixed_service_story_items(self):
        ...
        for fixed_service in self.fixed_services:
            table.append(
                [
                    fixed_service.date
                    and Paragraph(
                        fixed_service.date.strftime(line_item_date_format),
                        table_item_style,
                    ),
                    fixed_service.description
                    and Paragraph(fixed_service.description, table_item_style),
                    Paragraph(
                        "${:0.2f}".format(fixed_service.total), table_item_style_right
                    ),
                ]
            )
        ...
        table.append(
            [
                ...
                None,
                Paragraph(
                    "${:0.2f}".format(self.invoice.fixed_services_total),
                    table_total_style,
                ),
            ]
        )
    ...
    def get_expense_story_items(self):
        ...
        for expense in self.expenses:
            table.append(
                [
                    expense.date
                    and Paragraph(
                        expense.date.strftime(line_item_date_format), table_item_style
                    ),
                    expense.description
                    and Paragraph(expense.description, table_item_style),
        ...
        table.append(
            [
                ...
                None,
                Paragraph(
                    "${:0.2f}".format(self.invoice.expense_total), table_total_style
                ),
            ]
        )
    ...
    def get_payment_story_items(self):
        ...
        for payment in self.payments:
            table.append(
                [
                    payment.date
                    and Paragraph(
                        payment.date.strftime(line_item_date_format), table_item_style
                    ),
                    payment.description
                    and Paragraph(payment.description, table_item_style),
        ...
        table.append(
            [
                ...
                None,
                Paragraph(
                    "${:0.2f}".format(self.invoice.payment_total * -1),
                    table_total_style,
                ),
            ]
        )
    ...
    def get_credit_story_items(self):
        ...
        for credit in self.credits:
            table.append(
                [
                    credit.date
                    and Paragraph(
                        credit.date.strftime(line_item_date_format), table_item_style
                    ),
                    credit.description
                    and Paragraph(credit.description, table_item_style),
        ...
        table.append(
            [
                ...
                None,
                Paragraph(
                    "${:0.2f}".format(self.invoice.credit_total * -1), table_total_style
                ),
            ]
        )
    ...
    def build_pdf(self, output):
        ...
        def onLaterPages(canvas, doc):
            ...
            canvas.setFont("HelveticaNeue-Light", 16)
            canvas.setFillColor(colors.black)
            canvas.drawString(0.75 * inch, 10 * inch, "INVOICE")
            ...
            canvas.drawString(
                4.25 * inch,
                10.1 * inch,
                "Invoice number: {:} (continued)".format(self.invoice.id),
            )

            canvas.setStrokeColor(colors.grey)
            canvas.line(0.75 * inch, 9.9 * inch, 7.75 * inch, 9.9 * inch)
            ...
            if amount:
                table.append(
                    [
                        ...
                        Paragraph(
                            "${:0.2f}".format(amount * multiplier), table_total_style
                        ),
                    ]
                )
    ...
    def get_story(self):
        ...
        item_lists = [item for item in item_lists if len(item) > 0]
        ...
        for i, item_list in enumerate(map(list, item_lists)):
            if i == 0:  # First item
                ...
                yield Spacer(1, 0.5 * inch)

            elif i == item_count - 1:  # Last item
                yield KeepTogether(item_list)

            else:  # Middle items
                yield KeepTogether(item_list + [Spacer(1, 0.5 * inch)])
